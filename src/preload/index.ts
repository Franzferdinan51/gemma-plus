...
const api = {
  ...
  // Agent Teams
  agentTeamsSpawn: (task: string): Promise<string> =>
  ...
  // Agent Mesh
  meshStatus: (): Promise<{ connected: boolean; url: string }> =>
    ipcRenderer.invoke('mesh:status'),
  meshBroadcast: (message: string): Promise<string> =>
    ipcRenderer.invoke('mesh:broadcast', message),
  meshSend: (to: string, message: string): Promise<string> =>
    ipcRenderer.invoke('mesh:send', { to, message })
}
