...
import type { SetupStatus, ModelInfo } from '@shared/types'
  ...
  useEffect(() => {
    ...
    ;(async () => {
      unsub = window.api.onSetupStatus((status) => {
        setState((prev) => {
          if (status.stage === 'ready') {
            ...
            return { phase: 'ready', model: prev.phase === 'setup' ? prev.model : 'mlx-community/gemma-4-e4b-it-4bit' }
          }
          ...
          const model = prev.phase === 'setup' ? prev.model : 'mlx-community/gemma-4-e4b-it-4bit'
      ...
      const hasDefault = local.some(
        (m) => m === 'mlx-community/gemma-4-e4b-it-4bit' || m.startsWith('mlx-community/gemma-4-e4b-it-4bit:')
      )
      if (hasDefault) {
        ...
        if (hasMLX) {
          ...
          window.api.startSetup('mlx-community/gemma-4-e4b-it-4bit')
          return
        }
      }
      setState({
