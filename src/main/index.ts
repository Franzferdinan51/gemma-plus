...
import { startMeshService, stopMeshService, meshBroadcast, meshSendMessage, getMeshStatus } from './mesh'
import { runAgentTeamsCommand, getAgentTeamsStatus } from './agent-teams'
...
async function ensureMLXRunning(model: string): Promise<string> {
  let mlx = locateMLX()
  if (!mlx) throw new Error('Python 3.10–3.13 not found. Install via Homebrew: brew install python@3.13')
  ...
  send('setup:status', { stage: 'downloading-model', message: `Loading ${label}… (first run downloads the model)` })
  await startMLXServer(pythonToUse, model, (p) => {
    send('setup:status', { stage: 'downloading-model', message: p.message, progress: p.progress })
  })
  return pythonToUse
}
...
async function handleChat(req: ChatRequest, channel: string): Promise<void> {
  ...
  try {
    ...
    for (const m of req.messages) {
      baseMessages.push({ role: m.role as ChatMessage['role'], content: m.content })
      if (m.toolCalls) {
        for (const tc of m.toolCalls) {
          if (tc.result != null) {
            baseMessages.push({ role: 'tool', content: `Result of <action name="${tc.name}">: ${tc.result}` })
          }
        }
      }
    }
    ...
    // Get the right provider for this model
    const provider = getProviderForModel(req.model)

    // For MLX models, ensure server is running
    if (req.model.startsWith('mlx-community/')) {
      if (!mlxProvider.isServerRunning?.()) {
        await ensureMLXRunning(req.model)
      }
    }
    ...
    for (let round = 0; round < maxRounds; round++) {
      ...
      const writeLivePartial = (): void => {
        ...
        if (cleaned !== lastEmittedContent) {
          ...
          send('file:streaming', { conversationId: req.conversationId, path: livePath, content: cleaned, done: false })
        }
        livePending = wsWriteFile(req.conversationId, livePath, cleaned)
          .then(() => { send('workspace:changed', { conversationId: req.conversationId }) })
          .catch(() => { /* tolerate */ })
      ...
      const emitActivity = (): void => {
        ...
        lastActivityTs = now
        if (pendingAction) {
          emit({ type: 'activity', activity: { kind: 'tool', tool: pendingAction.name, target: pendingAction.target, chars: buffer.length } })
        } else {
          emit({ type: 'activity', activity: { kind: 'generating', chars: buffer.length } })
        }
      }

      streamLoop: for await (const chunk of provider.chatStream({ model: req.model, messages: baseMessages, signal: abort.signal })) {
        if (chunk.content) {
          if (firstToken) {
            firstToken = false
          ...
          if (!pendingAction) {
            ...
            if (openMatch) {
              ...
              pendingAction = { name, target: pathM?.[1] || urlM?.[1] || qM?.[1] || cmdM?.[1] }
            }
          ...
          while (true) {
            ...
            emit({ type: 'activity', activity: { kind: 'tool', tool: found.name, target: actionTarget(found.name, found.args) } })
            ...
            baseMessages.push({ role: 'tool', content: `[${hadError ? 'error' : 'ok'}] ${found.name}: ${result}` })
            executedAction = true
            if (livePath) {
              send('file:streaming', { conversationId: req.conversationId, path: livePath, content: lastEmittedContent, done: true })
            }
            pendingAction = null
            livePath = null
            liveContentStart = -1
        ...
        if (chunk.done) break streamLoop
      }

      if (!executedAction) {
        if (req.mode === 'code' && round === 0 && buffer.trim().length > 0) {
          if (emittedIdx < buffer.length) emit({ type: 'token', text: buffer.slice(emittedIdx) })
          baseMessages.push({ role: 'assistant', content: buffer })
          baseMessages.push({ role: 'user', content: 'Good plan. Now start building — emit a write_file action with the first file immediately.' })
          emit({ type: 'activity', activity: { kind: 'thinking', chars: 0 } })
          continue
        }
    ...
    emit({ type: 'activity', activity: { kind: 'idle' } })
    emit({ type: 'error', error: `Reached max tool rounds (${maxRounds}). Ask the model to finish up and try again.` })
  } catch (e) {
    emit({ type: 'activity', activity: { kind: 'idle' } })
...
app.whenReady().then(async () => {
  electronApp.setAppUserModelId('com.gemma-plus')
  ...
  startMeshService().catch((e) => console.log('[mesh] start failed:', e))

  session.defaultSession.setPermissionRequestHandler((_wc, permission, callback) => {
    if (permission === 'media' || permission === 'mediaKeySystem') { callback(true); return }
    callback(false)
  })
  ...
  ipcMain.handle('model:switch', async (_e, model: string) => {
    ...
    try {
      if (model.startsWith('mlx-community/')) {
        await stopMLXServer()
        if (!mlxPython) throw new Error('MLX Python path not available. Please restart the app.')
        await startMLXServer(mlxPython, model, (p) => {
          send('setup:status', { stage: 'downloading-model', message: p.message, progress: p.progress })
        })
      }
      send('setup:status', { stage: 'ready', message: 'Ready to chat.' })
    } catch (e) {
      send('setup:status', { stage: 'error', message: 'Model switch failed', error: (e as Error).message })
    }
  })
  ...
  ipcMain.handle('models:list-local', async () => { return listMLXModels() })
  ...
  ipcMain.handle('tools:list', async () => {
    return Object.values(TOOLS).map((t) => ({ name: t.name, description: t.description, mode: t.mode }))
  })

  ipcMain.handle('workspace:info', async (_e, conversationId: string) => {
    await ensureWorkspace(conversationId)
    return { conversationId, path: workspaceDir(conversationId), previewUrl: previewUrl(conversationId) }
  })
  ...
  // ---- Agent Teams ----
  ipcMain.handle('agent-teams:spawn', async (_e, task: string) => {
    return runAgentTeamsCommand('spawn', task)
  })
  ipcMain.handle('agent-teams:status', async () => {
    return getAgentTeamsStatus()
  })
  ipcMain.handle('agent-teams:delegate', async (_e, { agent, task }: { agent: string; task: string }) => {
  ...
  ipcMain.handle('mesh:send', async (_e, { to, message }: { to: string; message: string }) => {
    return meshSendMessage(to, message)
  })
